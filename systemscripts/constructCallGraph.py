#!/usr/bin/python3
'''
generates call graph of given python code file in dot format input for graphviz.

limitations:
* statically tried to figure out functions calls
* does not understand classes
* algorithm is naive and may not statically find all cases
'''
...
def usage():
	pe('Usage: %s <pythonSourceFiles...>', sys.argv[0])
...
def main():
	...
	# generate dot code 
	python_code = ''
	for pythonSourceFile in sys.argv[1:]:
		fd = open(pythonSourceFile, 'r')
		python_code += fd.read()
		fd.close()
	dot_code = generate_dot_code(python_code)
	# convert to png image file
	(ret, pngData) = runCmdGetOutput('dot -Tpng', stdinData=dot_code)
	if (ret == 0):
		pngFile = 'callGraph.png'
		fd = open(pngFile, 'w')
		fd.buffer.write(pngData)
		fd.close()
		# open with eog image viewer
		runCmd('eog %s' %(pngFile))
		os.unlink(pngFile)
	else:
		pe('error in converting to png image file')
